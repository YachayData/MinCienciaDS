...
my_df = df[['sin_vac_uci','una_dosis_uci', 'dos_dosis_uci', 'dos_dosis_comp_uci', 'dosis_unica_uci', 'dosis_unica_comp_uci', 'dosis_ref_comp_uci']]
...
my_pink = np.array([248, 0, 123])/256.
my_green = np.array([0, 179, 98])/256.
my_blue = np.array([0, 153, 230])/256.
...
while index2 < val2:
    if index2 == 0:
        plt.scatter(xvr[index], yvr[index], s = 80, color = my_blue, label='con pauta completa')
    else:
